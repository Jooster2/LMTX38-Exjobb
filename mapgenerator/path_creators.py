...
def depth_first(grid, s_pt, visited_cells):
    """
    DEPRECATED -- Use visit_complete instead
...
def path_to_finish(grid, s_pt, f_pt, visited_cells=[]):
    ...
    # Get the number of steps between start and finish.
    path_x, path_y = s_pt.calc_steps(f_pt)
    if abs(path_x) == abs(path_y):
        # If they are equal, it doesn't matter which one is main.
    ...
    # Get the side which is in the direction we want to go.
    if m_p == path_x:
    ...
    while current_cell is not f_pt:
        ...
        # Check if we are about to go past the finish point.
        if current_cell.get_neighbour(side).distance_to(f_pt) > distance:
            # If so, we switch the side we are currently working with.
            if side is main_side:
                side = sec_side
            else:
                side = main_side

        # Check for not visited (should never fail) and then knock wall.
        if current_cell.get_neighbour(side).is_not_visited():
            visited_cells.append(current_cell)
            current_cell.knock_wall(side)
            current_cell = current_cell.get_neighbour(side)
