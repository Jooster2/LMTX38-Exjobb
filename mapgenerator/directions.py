...
class Side(IntEnum):
    ...
    def biased_random(side_1, side_2, side_3=None, side_4=None,
        ...
        Set the sides corresponding p-values to some value, that is relative to
        the other sides p-value.
        Example:
            p_1 = 5, p_2 = 2, means there is a 5:2 ratio, or ~71.4% chance, that 
            side_1 is returned.
        """
    ...
    def convert_to_corner(side):
        """Return a tuple of Corners that together form the given Side."""
        if side is Side.UP:
...
class Corner(IntEnum):
    ...
    def convert_to_side(corners: tuple):
        """Return a Side if the two corners are next to each other, otherwise None."""
        if set(corners).issubset(Side.convert_to_corner(Side.UP)):
            return Side.UP
        elif set(corners).issubset(Side.convert_to_corner(Side.RIGHT)):
            return Side.RIGHT
        elif set(corners).issubset(Side.convert_to_corner(Side.DOWN)):
            return Side.DOWN
        elif set(corners).issubset(Side.convert_to_corner(Side.LEFT)):
            return Side.LEFT
